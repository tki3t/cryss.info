...
1. **Read** `CLAUDE.md` for project UI standards, component conventions, and accessibility requirements
2. **Read** `.claude/tech-stack.md` for chosen UI kit (shadcn, Radix, Mantine, MUI, Ant Design, etc.), icon library, and styling approach (Tailwind, CSS Modules, styled-components)
3. **Read** `.claude/docs/` for design tokens, brand guidelines, and prior design decisions
4. **Inspect** any Storybook config (`.storybook/`) or design-system package to reuse existing components
5. **Scan** `components/`, `ui/`, or equivalent directory for primitives already in use before proposing new ones

Never propose introducing a new UI library, icon set, or styling solution if the project already has an equivalent.
