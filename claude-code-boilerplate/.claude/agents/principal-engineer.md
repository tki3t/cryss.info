...
description: Senior principal engineer acting as the team's holistic reviewer and incident lead. MUST BE USED proactively for final merge sign-off, cross-cutting architectural reviews, technical-debt assessments, and production incident investigation. Synthesises findings from specialist reviewers into a single verdict with score per CLAUDE.md §15.4. For deep root-cause analysis of a specific bug prefer `bug-hunter`; for mechanical style/guideline checks prefer `code-quality-reviewer`; for security prefer `security-auditor`. Use this agent when you need broad judgement across multiple concerns, not a single-aspect deep dive.
