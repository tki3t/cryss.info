...
| 3 | `qa-engineer` | **Write FAILING tests FIRST** (TDD RED phase - MANDATORY) |
| 4 | `qa-engineer` | **Verify tests FAIL** for the right reasons |
| 5 | `fullstack-developer` / `vite-react-developer` | **Write MINIMUM code** to pass tests (TDD GREEN phase) |
| 6 | `fullstack-developer` / `vite-react-developer` | **Refactor** while keeping tests green (TDD REFACTOR phase) |
...
| 13 | `fullstack-developer` / `vite-react-developer` | If avg < 9.5: **Fix ALL issues** from ALL reviewers |
...
|-------|--------------|----------------|
| `principal-engineer` | Holistic: cross-cutting architecture + incident lens + merge verdict | Equal |
| `security-auditor` | OWASP vulnerabilities, auth, secrets, supply-chain | Equal |
| `bug-hunter` | Root-cause analysis, silent failures, defence-in-depth | Equal |
| `code-quality-reviewer` | DRY / KISS / SOLID, naming, CLAUDE.md compliance | Equal |
| `contracts-reviewer` | API / type / schema design, invariants, breaking changes | Equal |
| `test-coverage-reviewer` | Behavioural coverage quality (not line %) | Equal |
| `qa-engineer` | Test strategy, regression, edge cases | Equal |
| `uiux-designer` | UX, accessibility (WCAG 2.1 AA), design-system conformance | Equal |
| `solution-architect` | Scalability, system design, integration patterns | Equal |
| `historical-context-reviewer` | Git history, recurring issues, hotspots | Equal |

In practice the review is orchestrated by the `/review-local-changes` or `/review-pr` skill, which selects the applicable subset and filters findings through the Confidence × Impact threshold. See `.claude/AGENTS_INDEX.md` for the decision matrix.
...
Canonical agent list (matches `.claude/agents/*.md` `name:` exactly — Claude Code routes on this):

| Agent | Kind | Phase 0 Focus | Implementation / Review Focus |
|-------|------|---------------|-------------------------------|
| `business-analyst` | Plan | Requirements, domain docs, existing stories | Writes user stories + AC + MoSCoW priority |
| `solution-architect` | Plan | ADRs, existing architecture, tech-stack | Designs system / data model / API contracts |
| `uiux-designer` | Plan + Review | Design tokens, existing components, a11y spec | Reviews UX, WCAG 2.1 AA, design-system conformance |
| `seo-specialist` | Plan + Review + Build | Rendering mode, head management, i18n, sitemap | Technical + on-page SEO: meta, JSON-LD, canonical, hreflang, CWV impact on rankings |
| `fullstack-developer` | Build | Backend patterns, ORM, validation library | Implements backend / BFF / non-React frontends |
| `vite-react-developer` | Build | `vite.config`, router, data layer, UI kit | Implements Vite + React SPA components / hooks / routes |
| `game-tweak-engineer` | Build | Hook framework, Theos/MonkeyDev config, target binaries | C / C++ / ObjC / ObjC++ → `.dylib` tweaks; overlay menus, ESP, runtime hooks; low-overhead, responsive UI |
| `qa-engineer` | Build + Review | Existing test patterns + coverage | Writes FAILING tests first (TDD RED), then grows suite |
| `principal-engineer` | Review | Cross-cutting architecture, security hotspots | Holistic review, incident lead, final merge verdict |
| `bug-hunter` | Review | Critical paths, silent-failure patterns | Deep root-cause analysis + defence-in-depth |
| `security-auditor` | Review | OWASP-sensitive code paths | Security vulnerabilities (injection, auth, XSS, etc.) |
| `contracts-reviewer` | Review | Types, schemas, API contracts | Invariants, breaking changes, API stability |
| `code-quality-reviewer` | Review | Guideline files (CLAUDE.md / constitution.md) | DRY / KISS / SOLID / naming / style checklist |
| `test-coverage-reviewer` | Review | Critical paths needing tests | Behavioural coverage quality (not line %) |
| `historical-context-reviewer` | Review | Git blame, past PRs, hotspot files | Prevents repeating past mistakes |

Selection rule: **always pick the most specific agent**. If a dedicated specialist exists for the concern, use it; fall back to `principal-engineer` only for cross-cutting judgement calls. See `.claude/AGENTS_INDEX.md` for the full decision matrix.

**All agents participate in Phase 0 exploration based on their domain expertise.**

### 15.5.1 Skills (slash commands)

| Slash command | File | Purpose |
|---------------|------|---------|
| `/explore-codebase` | `.claude/skills/explore-codebase/SKILL.md` | Executes §15.1 Phase 0 systematically |
| `/run-quality-gates` | `.claude/skills/run-quality-gates/SKILL.md` | Runs §15.0.1 pre/post-impl gates (typecheck + lint + test + build) |
| `/feature-cycle` | `.claude/skills/feature-cycle/SKILL.md` | End-to-end TDD loop orchestrator covering §15.3 steps 5–14 |
| `/review-local-changes` | `.claude/skills/review-local-changes/SKILL.md` | Pre-commit multi-agent review of uncommitted diff |
| `/review-pr` | `.claude/skills/review-pr/SKILL.md` | GitHub PR inline review or `--json` for CI |

The two review skills share the same **Confidence × Impact Progressive Threshold** (81/50, 61/65, 41/75, 21/85, 0/95). `/feature-cycle` delegates review to `/review-local-changes` and therefore inherits the same threshold.

### 15.5.2 Language / framework presets

Language-specific snippets are **not** inlined into agent bodies (keeps context lean). They live in:

- `.claude/docs/backend-patterns.md` — TS / Python / Go / Rust / PHP backend snippets, loaded on demand by `fullstack-developer`
- `.claude/agents/vite-react-developer.md` — self-contained Vite + React reference
- `PHP_DEV_AGENTS_SKILLS.md` (repo root) — onboarding doc for PHP teams (Vietnamese)
- `VITE_REACT_DEV_AGENTS_SKILLS.md` (repo root) — onboarding doc for Vite + React teams (Vietnamese)
- `.claude/tech-stack.md` — the **single source of truth** every agent reads in STEP 1
...
│     - qa-engineer: Write FAILING tests      │
...
│     - fullstack-developer OR                │
│       vite-react-developer write MIN code   │
